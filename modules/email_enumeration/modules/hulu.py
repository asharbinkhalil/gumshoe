...
async def hulu_email(email):
    ...
    async with aiohttp.ClientSession(cookies=cookies, headers=headers) as session:
        async with session.get('https://signup.hulu.com/api/v3/accounts/status', params=params) as response:
            html = await response.text()

    text = '"status":"existing"'

    if text in html:
        print("There Exist a Hulu account with the email")
        return True
    else:
        return False
