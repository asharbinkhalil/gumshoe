...
async def discord_email(email):
    ...
    async with aiohttp.ClientSession(cookies=cookies, headers=headers) as session:
        async with session.post('https://discord.com/api/v9/auth/register', json=json_data) as response:
            html = await response.text()
            if 'Email is already registered.' in html:
                print("There Exist a discord account with the email")
                return True
            else:
                return False
